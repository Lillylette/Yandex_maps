...
class MapWindow(QMainWindow):
    def __init__(self):
        ...
        if not geocoder_key or not static_maps_key:
            ...
            sys.exit(1)
        self.api = YandexApiClient(geocoder_key, static_maps_key)
    ...
    def show_map(self):
        ...
        try:
            ...
            marker = (self.marker_lon, self.marker_lat) if self.has_marker else None
            pixmap_data = self.api.get_static_map(
                lon=float(lon),
                lat=float(lat),
            ...
            if pixmap_data:
                ...
                self.Map_itself.setText("Ошибка получения карты")
        except ValueError:
